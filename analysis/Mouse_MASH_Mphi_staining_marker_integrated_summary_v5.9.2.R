...
OUT <- file.path(ROOT, "Mouse_MASH_Mphi_RDS", "Mphi_StainingMarker_IntegratedSummary_v5.9.2")
...
write.csv(manifest, file.path(TAB, "00_marker_manifest_v5.9.2.csv"), row.names = FALSE)
...
whole$layer1_v592 <- as.character(whole@meta.data[[WHOLE_ANNOT]])
mphi$class_v592 <- as.character(mphi@meta.data[[MPHI_CLASS]])
mphi$sample_v592 <- as.character(mphi@meta.data[[SAMPLE_COL]])
mphi$condition_v592 <- canonical_condition(mphi@meta.data[[COND_COL]])
...
mphi$class_v592 <- factor(mphi$class_v592, levels = CLASS_LEVELS)

msg("Whole-cell cells: ", ncol(whole))
msg("Mphi cells: ", ncol(mphi))
print(table(mphi$class_v592, useNA = "ifany"))
print(table(mphi$sample_v592, useNA = "ifany"))
...
write.csv(audit, file.path(TAB, "01_gene_audit_wholecell_and_Mphi_v5.9.2.csv"), row.names = FALSE)
print(audit)
...
umap_manifest_whole_selected <- feature_pages(
  ...
  file.path(UMAP_WHOLE_DIR, "01A_WHOLECELL_UMAP_selected_v5.9.2.pdf"),
...
umap_manifest_whole_reference <- feature_pages(
  ...
  file.path(UMAP_WHOLE_DIR, "01B_WHOLECELL_UMAP_reference_v5.9.2.pdf"),
  "Whole-cell UMAP | reference",
  pt = 0.13, raster = TRUE
)

# 2. Whole-cell DotPlot + quantitative table
whole$layer1_v592 <- factor(whole$layer1_v592, levels = sort(unique(as.character(whole$layer1_v592))))

if (length(whole_sel)) {
  p <- DotPlot(whole, features = whole_sel, group.by = "layer1_v592", assay = ASSAY) +
    ...
    theme_classic(base_size = 8)
  save_pdf(p, file.path(FIG, "02A_WHOLECELL_Layer1_DotPlot_selected_v5.9.2.pdf"), 13, 8)
}

if (length(whole_ref)) {
  p <- DotPlot(whole, features = whole_ref, group.by = "layer1_v592", assay = ASSAY) +
    scale_color_gradient2(low = "#0033FF", mid = "#FFFFFF", high = "#FF1A1A", midpoint = 0) +
    RotatedAxis() + labs(title = "Whole-cell Layer1 | reference markers", x = NULL, y = NULL) +
    theme_classic(base_size = 8)
  save_pdf(p, file.path(FIG, "02B_WHOLECELL_Layer1_DotPlot_reference_v5.9.2.pdf"), 16, 8)
}

whole_sum <- summarize_by_group(whole, whole_all, whole$layer1_v592, "wholecell_Layer1", ASSAY)
write.csv(whole_sum, file.path(TAB, "02_wholecell_expression_positive_fraction_by_Layer1_v5.9.2.csv"), row.names = FALSE)
...
if (nrow(whole_sel_sum)) {
  p <- ggplot(whole_sel_sum, aes(factor(gene, levels = whole_sel), group, fill = positive_percent)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          plot.title = element_text(face = "bold", hjust = 0.5))
  save_pdf(p, file.path(FIG, "03_WHOLECELL_Layer1_positive_fraction_heatmap_selected_v5.9.2.pdf"), 13, 8)
}
...
umap_manifest_mphi_selected <- feature_pages(
  ...
  file.path(UMAP_MPHI_DIR, "04A_MPHI_UMAP_selected_v5.9.2.pdf"),
...
umap_manifest_mphi_reference <- feature_pages(
  ...
  file.path(UMAP_MPHI_DIR, "04B_MPHI_UMAP_reference_v5.9.2.pdf"),
  "Clean-B Mphi UMAP | reference",
  pt = 0.27, raster = FALSE
)

# 4. Mphi Sham vs Tx UMAP
mphi$ShamTx_v592 <- ifelse(mphi$condition_v592 %in% c("Sham", "Tx"), mphi$condition_v592, NA_character_)
mphi$ShamTx_v592 <- factor(mphi$ShamTx_v592, levels = c("Sham", "Tx"))
mphi_st <- subset(mphi, cells = colnames(mphi)[!is.na(mphi$ShamTx_v592)])

msg("Mphi Sham vs Tx UMAP...")
umap_manifest_shamtx_selected <- split_feature_pdf(
  mphi_st, mphi_sel, MPHI_UMAP, "ShamTx_v592",
  file.path(UMAP_SHAMTX_DIR, "05A_MPHI_UMAP_SHAM_vs_TX_selected_v5.9.2.pdf"),
  "Clean-B Mphi Sham vs Tx | selected",
  pt = 0.24, raster = FALSE
)

umap_manifest_shamtx_reference <- split_feature_pdf(
  mphi_st, mphi_ref, MPHI_UMAP, "ShamTx_v592",
  file.path(UMAP_SHAMTX_DIR, "05B_MPHI_UMAP_SHAM_vs_TX_reference_v5.9.2.pdf"),
...
write.csv(
  umap_manifest,
  file.path(TAB, "00b_UMAP_output_manifest_v5.9.2.csv"),
  row.names = FALSE
)
...
if (length(mphi_sel)) {
  p <- DotPlot(mphi, features = mphi_sel, group.by = "class_v592", assay = ASSAY) +
    scale_color_gradient2(low = "#0033FF", mid = "#FFFFFF", high = "#FF1A1A", midpoint = 0) +
    RotatedAxis() + labs(title = "Mphi subtype | selected markers", x = NULL, y = NULL) +
    theme_classic(base_size = 8)
  save_pdf(p, file.path(FIG, "06A_MPHI_subtype_DotPlot_selected_v5.9.2.pdf"), 13, 5.5)
}
if (length(mphi_ref)) {
  p <- DotPlot(mphi, features = mphi_ref, group.by = "class_v592", assay = ASSAY) +
    scale_color_gradient2(low = "#0033FF", mid = "#FFFFFF", high = "#FF1A1A", midpoint = 0) +
    RotatedAxis() + labs(title = "Mphi subtype | reference markers", x = NULL, y = NULL) +
    theme_classic(base_size = 8)
  save_pdf(p, file.path(FIG, "06B_MPHI_subtype_DotPlot_reference_v5.9.2.pdf"), 16, 5.5)
}

# 6. Violin by marker group
msg("Violin plots...")
pdf(file.path(FIG, "07A_MPHI_subtype_Violin_selected_by_group_v5.9.2.pdf"), 12, 7, useDingbats = FALSE)
for (grp in names(SELECTED)) {
  ...
  if (!length(gs)) next
  p <- VlnPlot(mphi, features = gs, group.by = "class_v592", pt.size = 0,
...
pdf(file.path(FIG, "07B_MPHI_subtype_Violin_reference_by_group_v5.9.2.pdf"), 12, 7, useDingbats = FALSE)
...
sub_sum <- summarize_by_group(mphi, mphi_all, mphi$class_v592, "Mphi_subtype", ASSAY)
write.csv(sub_sum, file.path(TAB, "03_Mphi_expression_positive_fraction_by_subtype_v5.9.2.csv"), row.names = FALSE)

# 8. allMphi_positive_fraction_by_sample
msg("allMphi_positive_fraction_by_sample...")
sample_sum <- summarize_by_group(mphi, mphi_all, mphi$sample_v592, "allMphi_sample", ASSAY) %>%
  rename(sample = group) %>%
  left_join(tibble(sample = mphi$sample_v592, condition = mphi$condition_v592) %>% distinct(),
            by = "sample")
write.csv(sample_sum, file.path(TAB, "04_allMphi_positive_fraction_by_sample_v5.9.2.csv"), row.names = FALSE)

preferred_samples <- c("STD_rep1", "STD", "CDHFD_rep1", "CDAHFD_rep1", "CDHFD", "CDAHFD",
                       "Sham1", "Sham20", "Tx17", "Tx5")
sample_levels <- unique(c(
  preferred_samples[preferred_samples %in% unique(mphi$sample_v592)],
  setdiff(sort(unique(mphi$sample_v592)), preferred_samples)
))
...
if (nrow(ss)) {
  p <- ggplot(ss, aes(factor(sample, levels = sample_levels), positive_fraction, group = 1)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          plot.title = element_text(face = "bold", hjust = 0.5))
  save_pdf(p, file.path(FIG, "08_allMphi_positive_fraction_by_sample_selected_v5.9.2.pdf"), 14, 11)
}
...
mean_z_long <- heatmap_z(
  ...
  file.path(FIG, "09_MPHI_subtype_mean_expression_Zscore_heatmap_v5.9.2.pdf"),
  18, 5.5
)
write.csv(mean_z_long, file.path(TAB, "05_MPHI_subtype_mean_expression_Zscore_long_v5.9.2.csv"), row.names = FALSE)

pos_z_long <- heatmap_z(
  sub_sum, "group", "positive_fraction", mphi_all, CLASS_LEVELS,
  "Mphi subtype RNA-positive fraction | row z-score",
  file.path(FIG, "10_MPHI_subtype_positive_fraction_Zscore_heatmap_v5.9.2.pdf"),
  18, 5.5
)
write.csv(pos_z_long, file.path(TAB, "06_MPHI_subtype_positive_fraction_Zscore_long_v5.9.2.csv"), row.names = FALSE)
...
sample_z_long <- heatmap_z(
  ...
  file.path(FIG, "11_allMphi_positive_fraction_by_sample_Zscore_heatmap_v5.9.2.pdf"),
  18, 6
)
write.csv(sample_z_long, file.path(TAB, "07_allMphi_positive_fraction_by_sample_Zscore_long_v5.9.2.csv"), row.names = FALSE)

# 12. Condition summary + Sham/Tx change table
msg("Condition summaries...")
cond_sum <- summarize_by_group(mphi, mphi_all, mphi$condition_v592, "allMphi_condition", ASSAY)
write.csv(cond_sum, file.path(TAB, "08_allMphi_expression_positive_fraction_by_condition_v5.9.2.csv"), row.names = FALSE)
...
write.csv(st, file.path(TAB, "09_allMphi_SHAM_vs_TX_marker_summary_v5.9.2.csv"), row.names = FALSE)
...
pdf(
  file.path(
    FIG,
    "12A_allMphi_Violin_SHAM_vs_TX_selected_v5.9.2.pdf"
  ),
  width = 10,
  height = 5.5,
  useDingbats = FALSE
...
pdf(
  file.path(
    FIG,
    "12B_allMphi_Violin_SHAM_vs_TX_reference_v5.9.2.pdf"
  ),
  width = 10,
  height = 5.5,
  useDingbats = FALSE
...
pdf(
  file.path(
    FIG,
    "13_MPHI_subtype_Violin_SHAM_vs_TX_selected_v5.9.2.pdf"
  ),
  width = 14,
  height = 6.5,
  useDingbats = FALSE
...
write.csv(
  ...
  file.path(
    TAB,
    "10_allMphi_positive_fraction_by_sample_SHAM_TX_only_v5.9.2.csv"
  ),
  row.names = FALSE
)
...
if (
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "14_allMphi_positive_fraction_by_sample_SHAM_TX_selected_v5.9.2.pdf"
    ),
    14,
    11
  )
...
if (
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "15_allMphi_positive_fraction_SHAM_TX_sample_heatmap_v5.9.2.pdf"
    ),
    18,
    5
  )
...
if (
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "16_allMphi_mean_expression_SHAM_TX_sample_heatmap_v5.9.2.pdf"
    ),
    18,
    5
  )
...
write.csv(
  ...
  file.path(
    TAB,
    "11_allMphi_SHAM_vs_TX_condition_raw_v5.9.2.csv"
  ),
  row.names = FALSE
)
...
if (
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "17A_allMphi_SHAM_vs_TX_positive_fraction_heatmap_v5.9.2.pdf"
    ),
    18,
    3.3
  )
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "17B_allMphi_SHAM_vs_TX_mean_expression_heatmap_v5.9.2.pdf"
    ),
    18,
    3.3
  )
...
write.csv(
  ...
  file.path(
    TAB,
    "12_allMphi_SHAM_vs_TX_delta_summary_v5.9.2.csv"
  ),
  row.names = FALSE
)
...
index <- tibble(
  ...
  file = c(
    "01A_WHOLECELL_UMAP_selected_v5.9.2.pdf",
    "01B_WHOLECELL_UMAP_reference_v5.9.2.pdf",
    "02A_WHOLECELL_Layer1_DotPlot_selected_v5.9.2.pdf",
    "02B_WHOLECELL_Layer1_DotPlot_reference_v5.9.2.pdf",
    "03_WHOLECELL_Layer1_positive_fraction_heatmap_selected_v5.9.2.pdf",
    "04A_MPHI_UMAP_selected_v5.9.2.pdf",
    "04B_MPHI_UMAP_reference_v5.9.2.pdf",
    "05A_MPHI_UMAP_SHAM_vs_TX_selected_v5.9.2.pdf",
    "05B_MPHI_UMAP_SHAM_vs_TX_reference_v5.9.2.pdf",
    "06A_MPHI_subtype_DotPlot_selected_v5.9.2.pdf",
    "06B_MPHI_subtype_DotPlot_reference_v5.9.2.pdf",
    "07A_MPHI_subtype_Violin_selected_by_group_v5.9.2.pdf",
    "07B_MPHI_subtype_Violin_reference_by_group_v5.9.2.pdf",
    "08_allMphi_positive_fraction_by_sample_selected_v5.9.2.pdf",
    "09_MPHI_subtype_mean_expression_Zscore_heatmap_v5.9.2.pdf",
    "10_MPHI_subtype_positive_fraction_Zscore_heatmap_v5.9.2.pdf",
    "11_allMphi_positive_fraction_by_sample_Zscore_heatmap_v5.9.2.pdf",
    "12A_allMphi_Violin_SHAM_vs_TX_selected_v5.9.2.pdf",
    "12B_allMphi_Violin_SHAM_vs_TX_reference_v5.9.2.pdf",
    "13_MPHI_subtype_Violin_SHAM_vs_TX_selected_v5.9.2.pdf",
    "14_allMphi_positive_fraction_by_sample_SHAM_TX_selected_v5.9.2.pdf",
    ...
    "00_marker_manifest_v5.9.2.csv",
    "01_gene_audit_wholecell_and_Mphi_v5.9.2.csv",
    "02_wholecell_expression_positive_fraction_by_Layer1_v5.9.2.csv",
    "03_Mphi_expression_positive_fraction_by_subtype_v5.9.2.csv",
    "04_allMphi_positive_fraction_by_sample_v5.9.2.csv",
    "05_MPHI_subtype_mean_expression_Zscore_long_v5.9.2.csv",
    "06_MPHI_subtype_positive_fraction_Zscore_long_v5.9.2.csv",
    "07_allMphi_positive_fraction_by_sample_Zscore_long_v5.9.2.csv",
    "08_allMphi_expression_positive_fraction_by_condition_v5.9.2.csv",
    "09_allMphi_SHAM_vs_TX_marker_summary_v5.9.2.csv",
    "10_allMphi_positive_fraction_by_sample_SHAM_TX_only_v5.9.2.csv",
    "11_allMphi_SHAM_vs_TX_condition_raw_v5.9.2.csv",
    "12_allMphi_SHAM_vs_TX_delta_summary_v5.9.2.csv"
  )
)
write.csv(index, file.path(OUT, "OUTPUT_INDEX_v5.9.2.csv"), row.names = FALSE)
...
metadata <- tibble(
  ...
  value = c("v5.9.2", WHOLE_RDS, MPHI_RDS, WHOLE_ANNOT, MPHI_CLASS,
            SAMPLE_COL, COND_COL, WHOLE_UMAP, MPHI_UMAP)
)
write.csv(metadata, file.path(LOG, "analysis_metadata_v5.9.2.csv"), row.names = FALSE)
capture.output(sessionInfo(), file = file.path(LOG, "sessionInfo_v5.9.2.txt"))
