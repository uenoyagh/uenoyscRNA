...
OUT_DIR <- file.path(
  DATA_ROOT,
  "Mouse_MASH_RDS",
  "WholeCell_MphiSubtype_StainingMarker_Specificity_v5.8.3"
)
...
if (!TARGET_KUPFFER_LABEL %in% celltypes) {
  stop(
    "Kupffer_Macrophage label not found."
  )
}
...
save_pdf(
  ...
  file.path(
    FIG_DIR,
    "01_FULL_CELL_candidate_UMAP_v5.8.3.pdf"
  ),
  width = 14,
  height = 4.2 *
    ceiling(
...
save_pdf(
  ...
  file.path(
    FIG_DIR,
    "02_LAYER1_DotPlot_all_candidates_v5.8.3.pdf"
  ),
  width = 12,
  height = 8
)
...
write.csv(
  ...
  file.path(
    TAB_DIR,
    "01_candidate_expression_by_Layer1_v5.8.3.csv"
  ),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(
    TAB_DIR,
    "02_Kupffer_specificity_metrics_v5.8.3.csv"
  ),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(
    TAB_DIR,
    "03_positive_cell_composition_by_Layer1_v5.8.3.csv"
  ),
  row.names = FALSE
)
...
save_pdf(
  ...
  file.path(
    FIG_DIR,
    "03_positive_cell_composition_all_candidates_v5.8.3.pdf"
  ),
  width = 12,
  height = 7
)
...
save_pdf(
  ...
  file.path(
    FIG_DIR,
    "04_LAYER1_positive_fraction_heatmap_v5.8.3.pdf"
  ),
  width = 12,
  height = 8
)
...
save_pdf(
  ...
  file.path(
    FIG_DIR,
    "05_LAYER1_mean_expression_heatmap_v5.8.3.pdf"
  ),
  width = 12,
  height = 8
)
...
write.csv(
  ...
  file.path(
    TAB_DIR,
    "04_FINAL_wholecell_Kupffer_specificity_ranking_v5.8.3.csv"
  ),
  row.names = FALSE
)
...
cat(
  "Whole-cell Kupffer specificity ranking v5.8.3\n"
)
...
if (
  ...
  write.csv(
    ...
    file.path(
      TAB_DIR,
      "05_reference_Mphi_marker_gene_audit_v5.8.3.csv"
    ),
    row.names = FALSE
  )
  ...
  if (length(ref_present)) {
    ...
    save_pdf(
      ...
      file.path(
        FIG_DIR,
        "07_REFERENCE_Cd68_Cd80_CD206_Cd86_M2markers_FULL_MPHI_UMAP_v5.8.3.pdf"
      ),
      width = 15,
      height = 7.5
    )
