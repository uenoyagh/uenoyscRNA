...
# ==============================================================================
# Mouse MASH macrophage RPCA reclustering analysis v4.3.0
# ==============================================================================
# Purpose:
...
#   /Users/uenoya/Projects/uenoyscRNA/analysis/Mouse_MASH_Mphi_STD_CDAHFD_Sham_Tx_RPCA_v4.3.0.R
#
# Important:
...
OUTPUT_DIR <- file.path(
  ...
  "STD_CDAHFD_Sham_Tx_Mphi_RPCA_Layer2_v4.3.0"
)
...
canonicalize_token <- function(x) {
  ...
  # In v4.3.0, lower-case letters were removed first, so "Sham" became "S"
...
# ------------------------------------------------------------------------------
# 8. Calculate cell-level scores and strict gates
# ------------------------------------------------------------------------------
score_df <- tibble(
  cell = colnames(mphi),
...
save_pdf("02_UMAP_Layer2_all_conditions_v4.2.2.pdf", p_layer2, 9.2, 7.2)
...
save_pdf("03A_UMAP_Layer2_split_STD_CDAHFD_v4.2.2.pdf", p_layer2_std_cdahfd, 13.0, 6.3)
...
save_pdf("04A_UMAP_condition_STD_CDAHFD_v4.2.2.pdf", p_condition_std_cdahfd, 8.2, 6.8)
...
save_pdf("03B_UMAP_Layer2_split_Sham_Tx_v4.2.2.pdf", p_layer2_sham_tx, 13.0, 6.3)
...
save_pdf("04B_UMAP_condition_Sham_Tx_v4.2.2.pdf", p_condition_sham_tx, 8.2, 6.8)
...
save_pdf("04C_UMAP_condition_all_v4.2.2.pdf", p_condition_all, 9.2, 7.2)
...
write.csv(
  cell_audit,
  file.path(OUTPUT_DIR, "Cell_level_Layer2_annotation_audit_v4.3.csv"),
  row.names = FALSE
)
...
write.csv(
  layer2_by_sample,
  file.path(OUTPUT_DIR, "Layer2_counts_proportions_by_sample_v4.3.csv"),
  row.names = FALSE
)
...
write.csv(
  layer2_by_condition,
  file.path(OUTPUT_DIR, "Layer2_counts_proportions_by_condition_v4.3.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(OUTPUT_DIR, "Strict_M1_M2_ratio_by_sample_v4.3.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(OUTPUT_DIR, "Strict_M1_M2_ratio_by_condition_v4.3.csv"),
  row.names = FALSE
)
...
save_pdf("05_Layer2_composition_by_sample_v4.2.2.pdf", p_composition_sample, 11.5, 6.5)
...
save_pdf("06_Strict_M1_M2_fraction_by_sample_v4.2.2.pdf", p_strict_m1_m2, 10.5, 6.5)
...
write.csv(
  cluster_audit,
  file.path(OUTPUT_DIR, "Layer2_by_cluster_and_condition_audit_v4.3.csv"),
  row.names = FALSE
)
...
write.csv(
  marker_manifest,
  file.path(OUTPUT_DIR, "Layer2_marker_manifest_v4.3.csv"),
  row.names = FALSE
)

run_manifest <- c(
  paste0("script_version: 4.2.0"),
  ...
  paste0("rpca_pca_reduction: ", RPCA_PCA_REDUCTION),
...
writeLines(run_manifest, file.path(OUTPUT_DIR, "run_manifest_v4.2.txt"))
