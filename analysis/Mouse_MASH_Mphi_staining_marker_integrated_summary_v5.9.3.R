...
#
# v5.9.3 UMAP-output repair
#   - UMAPs are now saved with ggsave() and verified immediately.
#   - Every gene gets an individual PDF.
#   - Multi-gene UMAPs are also saved as PAGE PDFs.
#   - Whole-cell FeaturePlots use raster=TRUE to avoid extremely large/slow vector PDFs.
#   - Clean-B Mphi UMAPs remain high-resolution vector plots.
#   - Mphi Sham-vs-Tx UMAPs are saved individually for every marker.
#   - A UMAP output manifest records file path, existence, and byte size.
#
...
save_umap_checked <- function(p, file, width, height) {
  ...
  if (!file.exists(file)) {
    stop("UMAP PDF was not created: ", file)
  }

  bytes <- file.info(file)$size

  if (!is.finite(bytes) || bytes < 1000) {
    stop("UMAP PDF is unexpectedly small/incomplete: ", file, " | bytes=", bytes)
  }

  msg("Saved UMAP: ", basename(file), " | ", bytes, " bytes")
...
feature_pages <- function(
  ...
  if (!length(genes)) return(tibble())

  base_dir <- dirname(file)
  stem <- tools::file_path_sans_ext(basename(file))
  indiv_dir <- file.path(base_dir, paste0(stem, "_INDIVIDUAL"))
  dir.create(indiv_dir, recursive = TRUE, showWarnings = FALSE)

  manifest <- list()
  kk <- 1L

  # Individual PDFs: most robust and easiest to inspect.
  for (g in genes) {
    p <- FeaturePlot(
      obj,
      features = g,
    ...
    out_g <- file.path(
      indiv_dir,
      paste0(sprintf("%02d", match(g, genes)), "_", g, ".pdf")
    )

    manifest[[kk]] <- save_umap_checked(p, out_g, 5.4, 4.8)
    kk <- kk + 1L
  }

  # Page PDFs: one file per page instead of relying on a multi-page graphics device.
  chunks <- split(seq_along(genes), ceiling(seq_along(genes) / per_page))

  for (i in seq_along(chunks)) {
    gs <- genes[chunks[[i]]]

    plist <- lapply(gs, function(g) {
      FeaturePlot(
        obj,
        features = g,
        reduction = reduction,
    ...
    p_page <- wrap_plots(plist, ncol = ncol) +
      plot_annotation(
        title = paste0(title, " | PAGE ", i, "/", length(chunks))
      )

    page_file <- file.path(
      base_dir,
      paste0(stem, "_PAGE", sprintf("%02d", i), ".pdf")
    )

    manifest[[kk]] <- save_umap_checked(
      p_page,
      page_file,
      13,
      4.1 * ceiling(length(gs) / ncol)
    )
    kk <- kk + 1L
  }
...
split_feature_pdf <- function(
  ...
  for (i in seq_along(genes)) {
    ...
    p <- p +
      plot_annotation(
        title = paste0(title, " | ", g)
      )

    out_g <- file.path(
      indiv_dir,
      paste0(sprintf("%02d", i), "_", g, "_Sham_vs_Tx.pdf")
    )

    manifest[[i]] <- save_umap_checked(p, out_g, 11, 5.2)
  }
...
OUT <- file.path(ROOT, "Mouse_MASH_Mphi_RDS", "Mphi_StainingMarker_IntegratedSummary_v5.9.3")
...
UMAP_DIR <- file.path(FIG, "UMAP")
UMAP_WHOLE_DIR <- file.path(UMAP_DIR, "WholeCell")
UMAP_MPHI_DIR <- file.path(UMAP_DIR, "Mphi")
UMAP_SHAMTX_DIR <- file.path(UMAP_DIR, "Mphi_Sham_vs_Tx")
for (d in c(UMAP_DIR, UMAP_WHOLE_DIR, UMAP_MPHI_DIR, UMAP_SHAMTX_DIR)) {
...
write.csv(manifest, file.path(TAB, "00_marker_manifest_v5.9.3.csv"), row.names = FALSE)
...
whole$layer1_v593 <- as.character(whole@meta.data[[WHOLE_ANNOT]])
mphi$class_v593 <- as.character(mphi@meta.data[[MPHI_CLASS]])
mphi$sample_v593 <- as.character(mphi@meta.data[[SAMPLE_COL]])
mphi$condition_v593 <- canonical_condition(mphi@meta.data[[COND_COL]])
...
mphi$class_v593 <- factor(mphi$class_v593, levels = CLASS_LEVELS)

msg("Whole-cell cells: ", ncol(whole))
msg("Mphi cells: ", ncol(mphi))
print(table(mphi$class_v593, useNA = "ifany"))
print(table(mphi$sample_v593, useNA = "ifany"))
...
write.csv(audit, file.path(TAB, "01_gene_audit_wholecell_and_Mphi_v5.9.3.csv"), row.names = FALSE)
print(audit)
...
# 1. Whole-cell UMAP
msg("Whole-cell UMAP...")
umap_manifest_whole_selected <- feature_pages(
  whole, whole_sel, WHOLE_UMAP,
  file.path(UMAP_WHOLE_DIR, "01A_WHOLECELL_UMAP_selected_v5.9.3.pdf"),
  "Whole-cell UMAP | selected",
  pt = 0.13, raster = TRUE
)

umap_manifest_whole_reference <- feature_pages(
  whole, whole_ref, WHOLE_UMAP,
  file.path(UMAP_WHOLE_DIR, "01B_WHOLECELL_UMAP_reference_v5.9.3.pdf"),
  "Whole-cell UMAP | reference",
  pt = 0.13, raster = TRUE
)

# 2. Whole-cell DotPlot + quantitative table
whole$layer1_v593 <- factor(whole$layer1_v593, levels = sort(unique(as.character(whole$layer1_v593))))

if (length(whole_sel)) {
  p <- DotPlot(whole, features = whole_sel, group.by = "layer1_v593", assay = ASSAY) +
    ...
    theme_classic(base_size = 8)
  save_pdf(p, file.path(FIG, "02A_WHOLECELL_Layer1_DotPlot_selected_v5.9.3.pdf"), 13, 8)
}

if (length(whole_ref)) {
  p <- DotPlot(whole, features = whole_ref, group.by = "layer1_v593", assay = ASSAY) +
    scale_color_gradient2(low = "#0033FF", mid = "#FFFFFF", high = "#FF1A1A", midpoint = 0) +
    RotatedAxis() + labs(title = "Whole-cell Layer1 | reference markers", x = NULL, y = NULL) +
    theme_classic(base_size = 8)
  save_pdf(p, file.path(FIG, "02B_WHOLECELL_Layer1_DotPlot_reference_v5.9.3.pdf"), 16, 8)
}

whole_sum <- summarize_by_group(whole, whole_all, whole$layer1_v593, "wholecell_Layer1", ASSAY)
write.csv(whole_sum, file.path(TAB, "02_wholecell_expression_positive_fraction_by_Layer1_v5.9.3.csv"), row.names = FALSE)
...
if (nrow(whole_sel_sum)) {
  p <- ggplot(whole_sel_sum, aes(factor(gene, levels = whole_sel), group, fill = positive_percent)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          plot.title = element_text(face = "bold", hjust = 0.5))
  save_pdf(p, file.path(FIG, "03_WHOLECELL_Layer1_positive_fraction_heatmap_selected_v5.9.3.pdf"), 13, 8)
}

# 3. Mphi UMAP
msg("Mphi UMAP...")
umap_manifest_mphi_selected <- feature_pages(
  mphi, mphi_sel, MPHI_UMAP,
  file.path(UMAP_MPHI_DIR, "04A_MPHI_UMAP_selected_v5.9.3.pdf"),
  "Clean-B Mphi UMAP | selected",
  pt = 0.27, raster = FALSE
)

umap_manifest_mphi_reference <- feature_pages(
  mphi, mphi_ref, MPHI_UMAP,
  file.path(UMAP_MPHI_DIR, "04B_MPHI_UMAP_reference_v5.9.3.pdf"),
  "Clean-B Mphi UMAP | reference",
  pt = 0.27, raster = FALSE
)

# 4. Mphi Sham vs Tx UMAP
mphi$ShamTx_v593 <- ifelse(mphi$condition_v593 %in% c("Sham", "Tx"), mphi$condition_v593, NA_character_)
mphi$ShamTx_v593 <- factor(mphi$ShamTx_v593, levels = c("Sham", "Tx"))
mphi_st <- subset(mphi, cells = colnames(mphi)[!is.na(mphi$ShamTx_v593)])

msg("Mphi Sham vs Tx UMAP...")
umap_manifest_shamtx_selected <- split_feature_pdf(
  mphi_st, mphi_sel, MPHI_UMAP, "ShamTx_v593",
  file.path(UMAP_SHAMTX_DIR, "05A_MPHI_UMAP_SHAM_vs_TX_selected_v5.9.3.pdf"),
  "Clean-B Mphi Sham vs Tx | selected",
  pt = 0.24, raster = FALSE
)

umap_manifest_shamtx_reference <- split_feature_pdf(
  mphi_st, mphi_ref, MPHI_UMAP, "ShamTx_v593",
  file.path(UMAP_SHAMTX_DIR, "05B_MPHI_UMAP_SHAM_vs_TX_reference_v5.9.3.pdf"),
  "Clean-B Mphi Sham vs Tx | reference",
  pt = 0.24, raster = FALSE
)

umap_manifest <- bind_rows(
  mutate(umap_manifest_whole_selected, panel = "WholeCell_selected"),
  mutate(umap_manifest_whole_reference, panel = "WholeCell_reference"),
  mutate(umap_manifest_mphi_selected, panel = "Mphi_selected"),
  mutate(umap_manifest_mphi_reference, panel = "Mphi_reference"),
  mutate(umap_manifest_shamtx_selected, panel = "Mphi_ShamTx_selected"),
  mutate(umap_manifest_shamtx_reference, panel = "Mphi_ShamTx_reference")
)

write.csv(
  umap_manifest,
  file.path(TAB, "00b_UMAP_output_manifest_v5.9.3.csv"),
  row.names = FALSE
)
...
if (length(mphi_sel)) {
  p <- DotPlot(mphi, features = mphi_sel, group.by = "class_v593", assay = ASSAY) +
    scale_color_gradient2(low = "#0033FF", mid = "#FFFFFF", high = "#FF1A1A", midpoint = 0) +
    RotatedAxis() + labs(title = "Mphi subtype | selected markers", x = NULL, y = NULL) +
    theme_classic(base_size = 8)
  save_pdf(p, file.path(FIG, "06A_MPHI_subtype_DotPlot_selected_v5.9.3.pdf"), 13, 5.5)
}
if (length(mphi_ref)) {
  p <- DotPlot(mphi, features = mphi_ref, group.by = "class_v593", assay = ASSAY) +
    scale_color_gradient2(low = "#0033FF", mid = "#FFFFFF", high = "#FF1A1A", midpoint = 0) +
    RotatedAxis() + labs(title = "Mphi subtype | reference markers", x = NULL, y = NULL) +
    theme_classic(base_size = 8)
  save_pdf(p, file.path(FIG, "06B_MPHI_subtype_DotPlot_reference_v5.9.3.pdf"), 16, 5.5)
}

# 6. Violin by marker group
msg("Violin plots...")
pdf(file.path(FIG, "07A_MPHI_subtype_Violin_selected_by_group_v5.9.3.pdf"), 12, 7, useDingbats = FALSE)
for (grp in names(SELECTED)) {
  ...
  if (!length(gs)) next
  p <- VlnPlot(mphi, features = gs, group.by = "class_v593", pt.size = 0,
...
pdf(file.path(FIG, "07B_MPHI_subtype_Violin_reference_by_group_v5.9.3.pdf"), 12, 7, useDingbats = FALSE)
...
sub_sum <- summarize_by_group(mphi, mphi_all, mphi$class_v593, "Mphi_subtype", ASSAY)
write.csv(sub_sum, file.path(TAB, "03_Mphi_expression_positive_fraction_by_subtype_v5.9.3.csv"), row.names = FALSE)

# 8. allMphi_positive_fraction_by_sample
msg("allMphi_positive_fraction_by_sample...")
sample_sum <- summarize_by_group(mphi, mphi_all, mphi$sample_v593, "allMphi_sample", ASSAY) %>%
  rename(sample = group) %>%
  left_join(tibble(sample = mphi$sample_v593, condition = mphi$condition_v593) %>% distinct(),
            by = "sample")
write.csv(sample_sum, file.path(TAB, "04_allMphi_positive_fraction_by_sample_v5.9.3.csv"), row.names = FALSE)

preferred_samples <- c("STD_rep1", "STD", "CDHFD_rep1", "CDAHFD_rep1", "CDHFD", "CDAHFD",
                       "Sham1", "Sham20", "Tx17", "Tx5")
sample_levels <- unique(c(
  preferred_samples[preferred_samples %in% unique(mphi$sample_v593)],
  setdiff(sort(unique(mphi$sample_v593)), preferred_samples)
))
...
if (nrow(ss)) {
  p <- ggplot(ss, aes(factor(sample, levels = sample_levels), positive_fraction, group = 1)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          plot.title = element_text(face = "bold", hjust = 0.5))
  save_pdf(p, file.path(FIG, "08_allMphi_positive_fraction_by_sample_selected_v5.9.3.pdf"), 14, 11)
}
...
mean_z_long <- heatmap_z(
  ...
  file.path(FIG, "09_MPHI_subtype_mean_expression_Zscore_heatmap_v5.9.3.pdf"),
  18, 5.5
)
write.csv(mean_z_long, file.path(TAB, "05_MPHI_subtype_mean_expression_Zscore_long_v5.9.3.csv"), row.names = FALSE)

pos_z_long <- heatmap_z(
  sub_sum, "group", "positive_fraction", mphi_all, CLASS_LEVELS,
  "Mphi subtype RNA-positive fraction | row z-score",
  file.path(FIG, "10_MPHI_subtype_positive_fraction_Zscore_heatmap_v5.9.3.pdf"),
  18, 5.5
)
write.csv(pos_z_long, file.path(TAB, "06_MPHI_subtype_positive_fraction_Zscore_long_v5.9.3.csv"), row.names = FALSE)
...
sample_z_long <- heatmap_z(
  ...
  file.path(FIG, "11_allMphi_positive_fraction_by_sample_Zscore_heatmap_v5.9.3.pdf"),
  18, 6
)
write.csv(sample_z_long, file.path(TAB, "07_allMphi_positive_fraction_by_sample_Zscore_long_v5.9.3.csv"), row.names = FALSE)

# 12. Condition summary + Sham/Tx change table
msg("Condition summaries...")
cond_sum <- summarize_by_group(mphi, mphi_all, mphi$condition_v593, "allMphi_condition", ASSAY)
write.csv(cond_sum, file.path(TAB, "08_allMphi_expression_positive_fraction_by_condition_v5.9.3.csv"), row.names = FALSE)
...
write.csv(st, file.path(TAB, "09_allMphi_SHAM_vs_TX_marker_summary_v5.9.3.csv"), row.names = FALSE)
...
pdf(
  file.path(
    FIG,
    "12A_allMphi_Violin_SHAM_vs_TX_selected_v5.9.3.pdf"
  ),
  width = 10,
  height = 5.5,
  useDingbats = FALSE
...
pdf(
  file.path(
    FIG,
    "12B_allMphi_Violin_SHAM_vs_TX_reference_v5.9.3.pdf"
  ),
  width = 10,
  height = 5.5,
  useDingbats = FALSE
...
pdf(
  file.path(
    FIG,
    "13_MPHI_subtype_Violin_SHAM_vs_TX_selected_v5.9.3.pdf"
  ),
  width = 14,
  height = 6.5,
  useDingbats = FALSE
...
write.csv(
  ...
  file.path(
    TAB,
    "10_allMphi_positive_fraction_by_sample_SHAM_TX_only_v5.9.3.csv"
  ),
  row.names = FALSE
)
...
if (
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "14_allMphi_positive_fraction_by_sample_SHAM_TX_selected_v5.9.3.pdf"
    ),
    14,
    11
  )
...
if (
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "15_allMphi_positive_fraction_SHAM_TX_sample_heatmap_v5.9.3.pdf"
    ),
    18,
    5
  )
...
if (
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "16_allMphi_mean_expression_SHAM_TX_sample_heatmap_v5.9.3.pdf"
    ),
    18,
    5
  )
...
write.csv(
  ...
  file.path(
    TAB,
    "11_allMphi_SHAM_vs_TX_condition_raw_v5.9.3.csv"
  ),
  row.names = FALSE
)
...
if (
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "17A_allMphi_SHAM_vs_TX_positive_fraction_heatmap_v5.9.3.pdf"
    ),
    18,
    3.3
  )
  ...
  save_pdf(
    ...
    file.path(
      FIG,
      "17B_allMphi_SHAM_vs_TX_mean_expression_heatmap_v5.9.3.pdf"
    ),
    18,
    3.3
  )
...
write.csv(
  ...
  file.path(
    TAB,
    "12_allMphi_SHAM_vs_TX_delta_summary_v5.9.3.csv"
  ),
  row.names = FALSE
)
...
index <- tibble(
  ...
  file = c(
    "01A_WHOLECELL_UMAP_selected_v5.9.3.pdf",
    "01B_WHOLECELL_UMAP_reference_v5.9.3.pdf",
    "02A_WHOLECELL_Layer1_DotPlot_selected_v5.9.3.pdf",
    "02B_WHOLECELL_Layer1_DotPlot_reference_v5.9.3.pdf",
    "03_WHOLECELL_Layer1_positive_fraction_heatmap_selected_v5.9.3.pdf",
    "04A_MPHI_UMAP_selected_v5.9.3.pdf",
    "04B_MPHI_UMAP_reference_v5.9.3.pdf",
    "05A_MPHI_UMAP_SHAM_vs_TX_selected_v5.9.3.pdf",
    "05B_MPHI_UMAP_SHAM_vs_TX_reference_v5.9.3.pdf",
    "06A_MPHI_subtype_DotPlot_selected_v5.9.3.pdf",
    "06B_MPHI_subtype_DotPlot_reference_v5.9.3.pdf",
    "07A_MPHI_subtype_Violin_selected_by_group_v5.9.3.pdf",
    "07B_MPHI_subtype_Violin_reference_by_group_v5.9.3.pdf",
    "08_allMphi_positive_fraction_by_sample_selected_v5.9.3.pdf",
    "09_MPHI_subtype_mean_expression_Zscore_heatmap_v5.9.3.pdf",
    "10_MPHI_subtype_positive_fraction_Zscore_heatmap_v5.9.3.pdf",
    "11_allMphi_positive_fraction_by_sample_Zscore_heatmap_v5.9.3.pdf",
    "12A_allMphi_Violin_SHAM_vs_TX_selected_v5.9.3.pdf",
    "12B_allMphi_Violin_SHAM_vs_TX_reference_v5.9.3.pdf",
    "13_MPHI_subtype_Violin_SHAM_vs_TX_selected_v5.9.3.pdf",
    "14_allMphi_positive_fraction_by_sample_SHAM_TX_selected_v5.9.3.pdf",
    ...
    "00_marker_manifest_v5.9.3.csv",
    "01_gene_audit_wholecell_and_Mphi_v5.9.3.csv",
    "02_wholecell_expression_positive_fraction_by_Layer1_v5.9.3.csv",
    "03_Mphi_expression_positive_fraction_by_subtype_v5.9.3.csv",
    "04_allMphi_positive_fraction_by_sample_v5.9.3.csv",
    "05_MPHI_subtype_mean_expression_Zscore_long_v5.9.3.csv",
    "06_MPHI_subtype_positive_fraction_Zscore_long_v5.9.3.csv",
    "07_allMphi_positive_fraction_by_sample_Zscore_long_v5.9.3.csv",
    "08_allMphi_expression_positive_fraction_by_condition_v5.9.3.csv",
    "09_allMphi_SHAM_vs_TX_marker_summary_v5.9.3.csv",
    "10_allMphi_positive_fraction_by_sample_SHAM_TX_only_v5.9.3.csv",
    "11_allMphi_SHAM_vs_TX_condition_raw_v5.9.3.csv",
    "12_allMphi_SHAM_vs_TX_delta_summary_v5.9.3.csv"
  )
)
write.csv(index, file.path(OUT, "OUTPUT_INDEX_v5.9.3.csv"), row.names = FALSE)
...
metadata <- tibble(
  ...
  value = c("v5.9.3", WHOLE_RDS, MPHI_RDS, WHOLE_ANNOT, MPHI_CLASS,
            SAMPLE_COL, COND_COL, WHOLE_UMAP, MPHI_UMAP)
)
write.csv(metadata, file.path(LOG, "analysis_metadata_v5.9.3.csv"), row.names = FALSE)
capture.output(sessionInfo(), file = file.path(LOG, "sessionInfo_v5.9.3.txt"))
