...
# ==============================================================================
# Mouse MASH macrophage analysis v4.1.0
# ==============================================================================
# Purpose:
...
#   /Users/uenoya/Projects/uenoyscRNA/analysis/Mouse_MASH_Mphi_STD_vs_CDAHFD_v4.1.0.R
#
# Important:
...
# Analysis targets (canonical labels used internally)
...
AUTO_MAX_EXCLUDED_FRACTION <- 0.02
AUTO_MIN_EXCLUDED_CELLS <- 3L
AUTO_MIN_CENTROID_SEPARATION_Z <- 2.00

# Strict classification parameters
SCORE_MARGIN_RESIDENT_MONOCYTE <- 0.20
SCORE_MARGIN_M1_M2 <- 0.15
MIN_RESIDENT_MARKERS <- 4L
MIN_MONOCYTE_CORE_MARKERS <- 2L
MIN_M1_MARKERS <- 4L
MIN_M2_MARKERS <- 4L
MIN_SPP1_TREM2_MARKERS <- 4L
...
resident_core <- c("Clec4f", "Timd4", "Vsig4", "Marco", "Cd5l")
resident_extended <- c(
  "Clec4f", "Timd4", "Vsig4", "Marco", "Cd5l", "Adgre1",
  "C1qa", "C1qb", "C1qc", "Fcgrt", "Slc40a1", "Hmox1", "Folr2", "Stab2"
)

# Monocyte definition emphasizes circulating/recently recruited monocyte markers.
monocyte_core <- c("Ly6c2", "Ccr2", "Plac8", "Sell", "S100a8", "S100a9")
monocyte_extended <- c(
  "Ly6c2", "Ccr2", "S100a8", "S100a9", "Plac8", "Chil3",
  "Sell", "Lair1", "Ctss", "Lyz2", "Ms4a7", "Fcgr1", "Ifitm6", "S100a10", "Lst1"
)
...
m1_markers <- c(
  ...
  "Cd80", "Cd86", "Nos2", "Il6", "Nfkbia", "Ier3", "Cxcl10", "Stat1", "Irf1"
)
...
spp1_trem2_anchor <- c("Spp1", "Trem2", "Gpnmb", "Cd9", "Lgals3")
...
# Hierarchical, mutually exclusive annotation.
# Priority:
#   1) SPP1/TREM2 MASH-associated
#   2) Resident Kupffer-like
#   3) Monocyte-like
#   4) strict inflammatory M1-like
#   5) strict pro-resolution M2-like
#   6) Other
#
# SPP1/TREM2 is evaluated first to avoid assigning lipid/scar-associated cells
# to M1 merely because they co-express inflammatory genes.
score_df <- score_df %>%
  mutate(
    spp1_trem2_strict =
      spp1_or_trem2_detected &
      spp1_trem2_anchor_n >= 2L &
...
p_layer2 <- make_umap_plot(
  ...
  title = "Mouse macrophage Layer2 annotation v4.1.0",
  point_size = 0.52
)
save_pdf("02_UMAP_Layer2_v4.1.pdf", p_layer2, 9.2, 7.2)
...
save_pdf("03_UMAP_Layer2_split_STD_CDAHFD_v4.1.pdf", p_layer2_split, 13.0, 6.3)
...
save_pdf("04_UMAP_condition_STD_CDAHFD_v4.1.pdf", p_condition, 8.2, 6.8)
...
write.csv(
  cell_audit,
  file.path(OUTPUT_DIR, "Cell_level_Layer2_annotation_audit_v4.1.csv"),
  row.names = FALSE
)
...
write.csv(
  layer2_by_sample,
  file.path(OUTPUT_DIR, "Layer2_counts_proportions_by_sample_v4.1.csv"),
  row.names = FALSE
)
...
write.csv(
  layer2_by_condition,
  file.path(OUTPUT_DIR, "Layer2_counts_proportions_by_condition_v4.1.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(OUTPUT_DIR, "Strict_M1_M2_ratio_by_sample_v4.1.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(OUTPUT_DIR, "Strict_M1_M2_ratio_by_condition_v4.1.csv"),
  row.names = FALSE
)
...
save_pdf("05_Layer2_composition_by_sample_v4.1.pdf", p_composition_sample, 11.5, 6.5)
...
save_pdf("06_Strict_M1_M2_fraction_by_sample_v4.1.pdf", p_strict_m1_m2, 10.5, 6.5)
...
write.csv(
  cluster_audit,
  file.path(OUTPUT_DIR, "Layer2_by_cluster_and_condition_audit_v4.1.csv"),
  row.names = FALSE
)
...
write.csv(
  marker_manifest,
  file.path(OUTPUT_DIR, "Layer2_marker_manifest_v4.1.csv"),
  row.names = FALSE
)

run_manifest <- c(
  paste0("script_version: 4.1.0"),
  ...
  paste0("annotation_hierarchy: SPP1/TREM2 > Resident > Monocyte > strict M1 > strict M2 > Other
strict_M1_M2_denominator: strict M1 + strict M2 only"),
  "sessionInfo:",
  paste(capture.output(sessionInfo()), collapse = "\n")
)
writeLines(run_manifest, file.path(OUTPUT_DIR, "run_manifest_v4.1.txt"))
