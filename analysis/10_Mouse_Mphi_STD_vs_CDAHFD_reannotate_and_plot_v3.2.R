...
mphi <- FindClusters(
  ...
  cluster.name = "mphi_cluster_v3.1",
...
cluster_score_table <- mphi@meta.data %>%
  mutate(
    mphi_cluster_v3.1 = as.character(
      mphi_cluster_v3
    )
  ) %>%
  group_by(
...
cluster_to_label <- setNames(
  ...
  cluster_score_table$mphi_cluster_v3.1
)

mphi$Layer2_annotation <- unname(
  cluster_to_label[
    as.character(
      mphi$mphi_cluster_v3.1
    )
  ]
)
...
save_plot(
  ...
  file.path(
    output_dir,
    "Mouse_Mphi_STD_vs_CDAHFD_combined_figure_v3.2.1"
  ),
  width = 22,
  height = 13
)
...
saveRDS(
  ...
  file.path(
    dir_rds,
    "Mouse_Mphi_STD_CDAHFD_reannotated_v3.2.rds"
  ),
  compress = FALSE
)
...
cat("\n")
cat("====================================================\n")
cat("Mouse Mphi reannotation and plotting v3.2 completed\n")
cat("====================================================\n")
cat("Cells analyzed :", ncol(mphi), "\n")
cat("Clusters       :", length(unique(mphi$mphi_cluster_v3.1)), "\n")
