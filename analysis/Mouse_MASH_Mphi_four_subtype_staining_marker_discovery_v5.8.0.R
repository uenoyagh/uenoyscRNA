...
OUTPUT_DIR <- file.path(DATA_ROOT, "Mouse_MASH_Mphi_RDS",
                        "Mphi_Subtype_StainingMarker_Discovery_v5.8.0")
...
write.csv(sample_long,
          file.path(TAB_DIR, "01_ALL_GENES_samplelevel_ShamTx_metrics_v5.8.0.csv"),
          row.names = FALSE)
...
write.csv(subtype_fraction,
          file.path(TAB_DIR, "02_subtype_fraction_by_sample_v5.8.0.csv"),
          row.names = FALSE)
...
for (subtype in TARGET_CLASSES) {
  ...
  ranking <- tibble(
    ...
    reference_gene = eligible_genes %in% REFERENCE_GENES[[subtype]]
  ) %>%
  ...
  write.csv(
    ranking,
    file.path(TAB_DIR, paste0("03_ALL_GENES_", slug, "_ranking_v5.8.0.csv")),
    row.names = FALSE
  )
  ...
  write.csv(
    top200,
    file.path(TAB_DIR, paste0("04_TOP200_", slug, "_staining_candidates_v5.8.0.csv")),
    row.names = FALSE
  )
  ...
  write.csv(
    reference_positions,
    file.path(TAB_DIR, paste0("05_REFERENCE_GENES_", slug, "_positions_v5.8.0.csv")),
    row.names = FALSE
  )
  ...
  save_pdf(
    p_rank,
    file.path(FIG_DIR, paste0("01_TOP40_", slug, "_ranking_v5.8.0.pdf")),
    8.5, 10
  )
  ...
  save_pdf(
    p_heat,
    file.path(FIG_DIR, paste0("02_TOP40_", slug, "_component_heatmap_v5.8.0.pdf")),
    10, 10
  )
  ...
  save_pdf(
    p_umap,
    file.path(UMAP_DIR, paste0("01_TOP12_", slug, "_FeaturePlot_v5.8.0.pdf")),
    14, 10
  )
  ...
  save_pdf(
    p_st,
    file.path(UMAP_DIR, paste0("02_TOP6_", slug, "_Sham_vs_Tx_FeaturePlot_v5.8.0.pdf")),
    11, 3.4 * TOP_N_SHAMTX
  )
}
...
write.csv(
  combined_ranking,
  file.path(TAB_DIR, "06_COMBINED_ALL_GENES_four_subtype_ranking_v5.8.0.csv"),
  row.names = FALSE
)
write.csv(
  combined_top200,
  file.path(TAB_DIR, "07_COMBINED_TOP200_four_subtype_candidates_v5.8.0.csv"),
  row.names = FALSE
)
write.csv(
  combined_reference,
  file.path(TAB_DIR, "08_COMBINED_reference_gene_positions_v5.8.0.csv"),
  row.names = FALSE
)
...
write.csv(
  cross_subtype,
  file.path(TAB_DIR, "09_TOP500_cross_subtype_specificity_source_v5.8.0.csv"),
  row.names = FALSE
)
...
if (length(reference_present)) {
  ...
  save_pdf(
    p_ref,
    file.path(UMAP_DIR, "03_REFERENCE_marker_panel_FeaturePlot_v5.8.0.pdf"),
...
metadata_out <- tibble(
  ...
  value = c(
    "v5.8.0", INPUT_RDS, CLASS_COLUMN, CONDITION_COLUMN, SAMPLE_COLUMN,
    CLUSTER_COLUMN, UMAP_USE, ASSAY_USE, W_FINAL_IDENTITY,
    W_FINAL_TREATMENT, GRID_NX, GRID_NY, MIN_CELLS_PER_GRID_BIN,
    MIN_DETECTION_FRACTION
  )
)
write.csv(metadata_out, file.path(LOG_DIR, "analysis_metadata_v5.8.0.csv"), row.names = FALSE)
capture.output(sessionInfo(), file = file.path(LOG_DIR, "sessionInfo_v5.8.0.txt"))

cat("\n============================================================\n")
cat("Four macrophage-subtype staining-marker discovery v5.8.0\n")
cat("============================================================\n")
