############################################################
## 10_Mouse_Mphi_STD_vs_CDAHFD_reannotate_and_plot_v3.1.R
##
...
output_dir <- paste0(
  "/Volumes/SSD990_uenoy/scRNA_MASLD_MASH_pseudobulk/",
  "Mouse_MASH_RDS/",
  "Mouse_Mphi_STD_vs_CDAHFD_v3.2.1"
)
...
## Resident-versus-Monocyte contrast threshold
resident_monocyte_contrast_threshold <- 0

## Provisional M1 clusters must exceed the Monocyte module by this margin.
## Otherwise, they are reassigned to Monocyte-like.
m1_minus_monocyte_minimum_margin <- 0.10

## UMAP display only: omit isolated lower-left population
ignore_lower_left_island_in_umap <- TRUE
lower_left_x_max <- -4.5
lower_left_y_max <- -6.5
ignore_extreme_left_singletons <- TRUE
extreme_left_x_max <- -10
extreme_left_y_max <- -5

random_seed <- 1234L
cluster_col <- "mphi_cluster_v3_2"
...
palette_name <- "Ueno MASH Macro Palette v1.1"

ueno_mash_macro_palette_v1_1 <- c(
  "Resident Kupffer-like Mphi"      = "#00C853",
  "Monocyte-like Mphi"              = "#F4A300",
...
mphi_subtype_levels <- names(ueno_mash_macro_palette_v1_1)
...
make_umap_plot <- function(
  ...
  p <- ggplot(
    ...
    scale_colour_manual(
      values = ueno_mash_macro_palette_v1_1,
...
## Dedicated Resident-versus-Monocyte contrast
mphi$Resident_Monocyte_contrast <-
  mphi$Module_Resident_Kupffer -
  mphi$Module_Monocyte
...
cluster_score_table <- mphi@meta.data %>%
  ...
  summarise(
    ...
    Resident_Monocyte_contrast = median(
      Resident_Monocyte_contrast,
      na.rm = TRUE
    ),
    .groups = "drop"
  )
...
## Two-stage Resident/Monocyte correction
resident_or_monocyte <- top_signature %in% c(
  "Resident_Kupffer",
  "Monocyte"
)

cluster_label[
  resident_or_monocyte &
    cluster_score_table$Resident_Monocyte_contrast >
    resident_monocyte_contrast_threshold
...
## Strict M1 gate against Monocyte contamination
cluster_score_table$M1_minus_Monocyte <-
  cluster_score_table$Module_Inflammatory_M1 -
  cluster_score_table$Module_Monocyte

provisional_m1 <- cluster_label ==
  "Inflammatory M1-like Mphi"

cluster_label[
  provisional_m1 &
    cluster_score_table$M1_minus_Monocyte <
    m1_minus_monocyte_minimum_margin
] <- "Monocyte-like Mphi"
...
cluster_to_label <- setNames(
  ...
  cluster_score_table$.cluster_id
)

mphi$Layer2_annotation <- unname(
  cluster_to_label[
    as.character(
      mphi@meta.data[[cluster_col]]
    )
  ]
)
...
umap_df_all <- left_join(
  umap_df_all,
  md,
  by = "cell"
)

umap_df_all$Condition_v3 <- factor(
  umap_df_all$Condition_v3,
  levels = conditions_keep
)

umap_df_all$Layer2_annotation <- factor(
  umap_df_all$Layer2_annotation,
  levels = mphi_subtype_levels
)

umap_df_all$Ignore_in_UMAP <- FALSE

if (isTRUE(ignore_lower_left_island_in_umap)) {
  umap_df_all$Ignore_in_UMAP <-
    umap_df_all$Ignore_in_UMAP |
    (umap_df_all$MphiUMAP_1 < lower_left_x_max &
       umap_df_all$MphiUMAP_2 < lower_left_y_max)
}

if (isTRUE(ignore_extreme_left_singletons)) {
  umap_df_all$Ignore_in_UMAP <-
    umap_df_all$Ignore_in_UMAP |
    (umap_df_all$MphiUMAP_1 < extreme_left_x_max &
       umap_df_all$MphiUMAP_2 < extreme_left_y_max)
}

umap_df_plot <- umap_df_all %>%
  filter(!Ignore_in_UMAP)

ignored_umap_cells <- umap_df_all %>%
  filter(Ignore_in_UMAP)
...
fwrite(
  ...
  file.path(
    dir_table,
    "MphiUMAP_coordinates_and_metadata_all_cells.csv"
  )
)
...
fwrite(
  ...
  file.path(
    dir_table,
    "UMAP_display_ignored_cells.csv"
  )
)

write_log(
  paste0(
    "Cells ignored in UMAP display only: ",
    nrow(ignored_umap_cells)
  )
)
...
save_plot(
  ...
  file.path(
    dir_umap,
    "UMAP_Mphi_STD_vs_CDAHFD_shared_coordinates_v3.1"
  ),
  width = 14,
  height = 7
)
...
sample_levels <- unique(
  as.character(
    umap_df_plot$Sample_v3
  )
)

umap_df_plot$Sample_v3 <- factor(
  umap_df_plot$Sample_v3,
...
save_plot(
  ...
  file.path(
    dir_module,
    "ModuleScore_five_signatures_STD_vs_CDAHFD_v3.1"
  ),
  width = 12,
  height = 8
)
...
save_plot(
  ...
  file.path(
    dir_count,
    "Mphi_subtype_fraction_STD_vs_CDAHFD_v3.1"
  ),
  width = 7.5,
  height = 7
)
...
############################################################
## 21. RESTRICTED M1/M2 COUNTS, FRACTIONS AND RATIO
############################################################

## Only the final strict M1 and M2 populations are used here.
## Resident Kupffer-like, Monocyte-like, SPP1/TREM2 and Other cells
## are excluded from both numerator and denominator.

m1_m2_restricted <- umap_df_all %>%
...
m1_m2_counts <- m1_m2_restricted %>%
  ...
  mutate(
    Restricted_M1_M2_total =
      sum(
        Cell_count
      ),

    Fraction_within_M1_M2 =
      ifelse(
        Restricted_M1_M2_total > 0,
        Cell_count /
          Restricted_M1_M2_total,
        NA_real_
      ),

    Percent_within_M1_M2 =
      100 *
      Fraction_within_M1_M2
  ) %>%
...
fwrite(
  ...
  file.path(
    dir_table,
    "M1_M2_restricted_counts_and_fractions.csv"
  )
)
...
p_m1_m2_count <- ggplot(
  ...
  labs(
    title =
      "Strict M1 and M2 macrophage counts",

    subtitle =
      "Resident, Monocyte, SPP1/TREM2 and Other excluded",

    x =
      NULL,

    y =
      "Cell count within strict M1/M2 compartment"
  ) +
...
save_plot(
  ...
  file.path(
    dir_count,
    "M1_M2_restricted_cell_counts_STD_vs_CDAHFD_v3.2"
  ),
  width = 7.5,
  height = 6.5
)

p_m1_m2_fraction <- ggplot(
  m1_m2_counts,
  aes(
    x = Condition_v3_1,
    y = Fraction_within_M1_M2,
    fill = M1_M2_class
  )
) +
  ...
  labs(
    title =
      "M1/M2 composition within strict M1 + M2 fraction",

    subtitle =
      "Denominator = strict M1 cells + M2 cells only",

    x =
      NULL,

    y =
      "Fraction within strict M1 + M2 compartment"
  ) +
...
save_plot(
  ...
  file.path(
    dir_ratio,
    "M1_M2_restricted_fraction_STD_vs_CDAHFD_v3.2"
  ),
  width = 7.5,
  height = 6.5
)
...
fwrite(
  ...
  file.path(
    dir_table,
    "M1_M2_restricted_ratio_and_fractions.csv"
  )
)
...
p_ratio <- ggplot(
  ...
  labs(
    title =
      "M2/M1 ratio within strict M1 + M2 fraction",
...
save_plot(
  ...
  file.path(
    dir_ratio,
    "M2_M1_restricted_log2_ratio_STD_vs_CDAHFD_v3.2"
  ),
  width = 7.5,
  height = 6.5
)
...
save_plot(
  ...
  file.path(
    dir_ratio,
    "SPP1_TREM2_Mphi_percentage_STD_vs_CDAHFD_v3.1"
  ),
  width = 7,
  height = 6.5
)
...
save_plot(
  ...
  file.path(
    dir_module,
    "DotPlot_marker_validation_v3.1"
  ),
  width = 16,
  height = 7
)
...
save_plot(
  ...
  file.path(
    output_dir,
    "Mouse_Mphi_STD_vs_CDAHFD_combined_figure_v3.2.1.1"
  ),
  width = 22,
  height = 13
)
...
saveRDS(
  ...
  file.path(
    dir_rds,
    "Mouse_Mphi_STD_CDAHFD_reannotated_v3.2.1.rds"
  ),
  compress = FALSE
)
...
saveRDS(
  ...
  file.path(
    dir_table,
    "Ueno_MASH_Macro_Palette_v1.1.rds"
  )
)
...
writeLines(
  ...
  file.path(
    dir_table,
    "Ueno_MASH_Macro_Palette_v1.1.R"
  )
)
...
cat("\n")
cat("====================================================\n")
cat("Mouse Mphi reannotation and plotting v3.2.1 completed\n")
cat("====================================================\n")
cat("Cells analyzed :", ncol(mphi), "\n")
cat("Clusters       :", length(unique(mphi@meta.data[[cluster_col]])), "\n")
