...
OUTPUT_DIR <- file.path(
  "/Volumes/SSD990_uenoy/scRNA_MASLD_MASH_pseudobulk/Mouse_MASH_RDS",
  "RDS3_ShamTx_HSC_IL10_ECM_v1.1.0"
)
...
# ------------------------------------------------------------------------------
# 5. Extract the HSC_Mesenchymal parent population
# ------------------------------------------------------------------------------
HSC_PARENT_LABEL <- "HSC_Mesenchymal"

if (!(HSC_PARENT_LABEL %in% unique(obj_st$celltype_internal))) {
  stop(
    "HSC parent label was not found: ", HSC_PARENT_LABEL, "\n",
    "Review: ",
    file.path(TABLE_DIR, "00_available_celltype_labels_ShamTx.csv")
  )
}

hsc_cells <- colnames(obj_st)[
  obj_st$celltype_internal == HSC_PARENT_LABEL
]

hsc <- subset(obj_st, cells = hsc_cells)
message_time("Extracted HSC_Mesenchymal cells: ", ncol(hsc))
...
# Classify each HSC_Mesenchymal cell by the relative aHSC and qHSC scores.
# This is a data-driven state label because RDS3 contains one combined HSC label.
hsc$hsc_state_margin <- hsc$aHSC_score - hsc$qHSC_score
hsc$hsc_state_internal <- ifelse(
  hsc$hsc_state_margin > 0,
  "aHSC",
  "qHSC"
)
...
write.csv(
  hsc@meta.data %>%
    ...
    count(condition_internal, sample_internal, hsc_state_internal, name = "n_cells"),
  file.path(TABLE_DIR, "00B_HSC_state_counts_after_marker_classification.csv"),
  row.names = FALSE
)
...
saveRDS(
  obj_st,
  file.path(RDS_DIR, "RDS3_allcells_ShamTx_IL10_receptor_scored_v1.1.0.rds"),
  compress = FALSE
)

saveRDS(
  hsc,
  file.path(RDS_DIR, "RDS3_HSC_ShamTx_ECM_IL10_scored_v1.1.0.rds"),
  compress = FALSE
)
...
write_excel_sheets(
  ...
  file.path(TABLE_DIR, "RDS3_ShamTx_HSC_ECM_IL10_summary_v1.1.0.xlsx")
)
...
writeLines(
  log_lines,
  file.path(LOG_DIR, "run_manifest_v1.1.0.txt")
)
