# ============================================================
# uenoyscRNA Mouse macrophage RPCA cluster→Layer workflow v2.0.0
# ============================================================
...
apply_manual_override_v200 <- function(provisional, override_file) {
  ...
  bad <- setdiff(unique(ov$manual_layer1[nzchar(ov$manual_layer1)]), valid)
  if (length(bad)>0) stop("Invalid manual_layer1 values: ", paste(bad,collapse=", "))
  map <- setNames(ov$manual_layer1, ov$cluster)
